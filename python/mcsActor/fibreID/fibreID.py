...
def check(aCobras,unaCobras,aPoints,unaPoints,potCobraMatch,potPointMatch,centers,goodIdx,arms,points):
    ...
    for i,elem in enumerate(potCobraMatch):
        ...
        if(len(elem) == 0):
            ...
            print("line ",centers[elem[0],1],centers[elem[0],2],points[i,1],points[i,2],"#color=yellow",file=ff1)
            if(dd > arms[elem[0]]+2):
                print("BB",i,elem,dd)
...
def getCentroids(data,threshFind,threshCent,boxFind,boxCent,nmin,nmax,maxIt):
    ...
    a=centroid.centroid_only(data.astype('<i4'),0,0,threshFind,threshCent,boxFind,boxCent,nmin,nmax,maxIt,0)
...
def getCalibBench(xmlFile,broken):
    ...
    centersAll=np.array([np.arange(nCobrasAll),pfic.calibModel.centers.real,pfic.calibModel.centers.imag]).T
...
def testFirstPassSimple():
    ...
    potCobraMatch,aCobras,unaCobas=firstPassNew(points,nPoints,nCobras,centers,armLength*np.ones((nCobras)),goodIdx)
