...
def getAllCentroids(files, outfile, thresh1, thresh2, fwhmx, fwhmy, boxFind, boxCent, nmin, nmax, maxIt):
    ...
    for file in files:
        ...
        # centroid
        x, y, fx, fy, back, peak, qual = getCentroids(
            image, thresh1, thresh2, fwhmx, fwhmy, boxFind, boxCent, nmin, nmax, maxIt)
...
def getAutoThresh(image, xr, yr, mx, my):
    ...
    xyout = CoordTransp.CoordinateTransform(
        xyin, 0, 'pfi_mcs_wofe', inr=-angle*180/np.pi, cent=np.array([[xr], [yr]]))
