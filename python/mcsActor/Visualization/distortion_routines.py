...
def plotDistortion(cc, c1, pts1, pts2, diffx, diffy, fxs, fys, peaks, limit, prefix):
    ...
    plt.clf()
    fig = plt.figure(figsize=(18, 16), dpi=80, facecolor='w', edgecolor='k')
    plt.scatter(np.ravel(pts2[0, ind, 0]), np.ravel(pts2[0, ind, 1]),
                marker='s', c=cc[ind], cmap='plasma', s=200)
    plt.colorbar()
    ...
    fig = plt.figure(figsize=(18, 16), dpi=80, facecolor='w', edgecolor='k')
    plt.scatter(np.ravel(pts2[0, ind, 0]), np.ravel(pts2[0, ind, 1]),
                marker='s', c=c1[ind]*4, cmap='plasma', s=200)
    plt.colorbar()
...
def plotVal(xs, ys, val, limit, plotrange, titl, prefix, suffix):
    ...
    if(plotrange != None):
        sc = plt.scatter(xs[ind], ys[ind], c=val[ind], marker="s", cmap='Purples',
                         lw=0, s=200, vmin=plotrange[0], vmax=plotrange[1])
