...
static const struct usb_device_id id_table[] = {
	...
	{ USB_DEVICE(PL2303_VENDOR_ID, PL2303_PRODUCT_ID2) },
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case TIOCGSERIAL:
		...
		return 0;
	case GP0_OUTPUT_ENABLE:
			data = (void __user *) arg;
			if (data == NULL)
				break;
			if (copy_from_user(&kdata, data, sizeof(__u16))) {
				retval = -EFAULT;
				goto err_out;
			}			
			result=usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev,0),	
			       VENDOR_READ_REQUEST, VENDOR_READ_REQUEST_TYPE, 
			       0x0081, 0, &buf, sizeof(__u8), 0);	
			if (result<0) {
				retval = result;
				goto err_out;
			}
	...
	case GP0_GET_VALUE:
			...
			if (buf & 0x40)
				...
				kdata = 0;
			if (copy_to_user(data, &kdata, sizeof(__u16))) {
				retval = -EFAULT;
				goto err_out;
			}				
			break;		
